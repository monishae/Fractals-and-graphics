...
class CvFractalGrammars extends Canvas {
	...
	double xLast, yLast, dir, rotation, dirStart, fxStart, fyStart, lengthFract, reductFact, thickFact;
	...
	public void paint(Graphics g) {
		...
		maxX = d.width - 1;
		maxY = d.height - 1;
		xLast = fxStart * maxX; // starting at center X
		yLast = fyStart * maxY;
		...
		for (int i = 0; i < instruction.length(); i++) {
			...
			case 'F': // Step forward and draw
				...
				if (depth == 0) {
					...
					if (i == 5) {
/* Set Coordinates and rotation of the fractal*/
						g.setColor(Color.GREEN);
						g.fillOval(iX(xLast) - 10, iY(yLast) - 3, 20, 6); 											// tree
